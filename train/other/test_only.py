...
class ContrastiveTypeLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        loss_type_contrastive = torch.mean((1 - label) * torch.pow(cos_distance, 2) + label * torch.pow(
            torch.clamp(self.margin - cos_distance, min=0.0), 2))
...
def periodic_test(test_iter, model, criterion, config, sum_epoch):
    ...
    print('{:.5f}'.format(plmt[0]), '\t{:.5f}'.format(plmt[1]), '\t{:.5f}'.format(plmt[2]),
          '\t\t{:.5f}'.format(plmt[3]),
          '\t\t{:.5f}'.format(plmt[4]), '\t{:.5f}'.format(plmt[5]), '\t{:.5f}'.format(plmt[6]),
          '\t{:.0f}'.format(plmt[7]),
...
def train_model(train_iter, valid_iter, test_iter, model, optimizer, criterion, contras_criterion,
                contras_type_criterion, config, iter_k):
    best_acc = 0
    best_performance = 0
    train_batch_loss = 0
    for epoch in range(1, config.epoch + 1):
        ...
        print(
            f"Train - Epoch[{epoch}] - loss: {train_epoch_loss / (len(train_iter) // config.batch_size)} | ACC: {(train_correct_num / train_total_num) * 100:.4f}%({train_correct_num}/{train_total_num})")
...
def draw_umap(repres_list, label_list):
    ...
    plt.savefig(config.load_model + '.svg', dpi=500)
    print('Figure saved!')
...
def model_eval(data_iter, model, criterion, config):
    ...
    with torch.no_grad():
        ...
        for batch in data_iter:
            ...
            # Umap
            tmp_pooler_output = pooler_output.view(pooler_output.size(-1))
            tmp_type = type.view(type.size(-1))
            # print(tmp_pooler_output.size())
            # print(tmp_type.size())
            pooler_list.append(tmp_pooler_output.cpu().numpy())
            type_list.append(tmp_type.cpu().numpy().item())
    ...
    # Umap
    # draw_umap(pooler_list, type_list)
...
def train_test(train_iter, test_iter, config):
    ...
    # model_path = './case_model/Case_AUC=0.894; MCC=0.455; F1=0.473.pl'
    # model_path = 'RNA_AUC=0.843; MCC=0.430; F1=0.513; 1.0-1.0-1.0_27.0_1.pl'
